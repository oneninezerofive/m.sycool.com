	...
	<div class="home">
		...
		<div class="bag_box">
			<ul class="lis clearfix">
				<li v-for="(item,index) in scroll" :key="index" :data-id="item.urlType" @click="golist(item.urlType)">
		...
		<div class="foot_btn">
			<ul>
				<li v-for="(item,index) in foot" :key="index" @click="active(index)" :class="{active:index == 0}">
